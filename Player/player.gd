...
#Guarda x/y que intenta mirar el player
var _look := Vector2.ZERO

# Guarda la dirección del jugador cuando ataca
...
func _physics_process(delta: float) -> void:
	...
	# Handle jump. (NO SALTA NUESTRO PJ POR AHORA)
...
func handle_idle_physics_frame(delta:float, direction:Vector3) -> void:
		if not rig.is_idle() and rig.is_dashing():
			return
